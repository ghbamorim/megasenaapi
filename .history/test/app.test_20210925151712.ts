...
test("result 123", async () => {
  const res = await request(app).get("/results/123").body;
  expect(res.coluna_1).not.toBeNaN;
  expect(res.coluna_2).not.toBeNaN;
  expect(res.coluna_3).not.toBeNaN;
  expect(res.coluna_4).not.toBeNaN;
  expect(res.coluna_5).not.toBeNaN;
  expect(res.coluna_6).not.toBeNaN;
});
/*
